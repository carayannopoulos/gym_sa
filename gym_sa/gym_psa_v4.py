...
class ParallelAnnealer:
    def __init__(
        ...
        print(f"len(self.remotes) = {len(self.remotes)}")
        print(f"len(self.processes) = {len(self.processes)}")
        for i, (remote, proc) in enumerate(zip(self.remotes, self.processes)):
            print(
                f"Chain {i}: remote fd={remote.fileno()}, process pid={proc.pid}, alive={proc.is_alive()}"
            )
    ...
    def set_state_remote_chains(self, states: List[Any], objectives: List[float]):
        ...
        self.log_debug(
            f"Number of states: {len(states)}, remotes: {len(self.remotes)}, processes: {len(self.processes)}"
        )
        ...
        outputs = []
        for i, (remote, process, state, objective) in enumerate(
            zip(self.remotes, self.processes, states, objectives)
        ):
            ...
            t = time.time()
            self.log_debug(
                f"{i}, about to send to remote fd={remote.fileno()} process: {process.is_alive()} remote: {remote.closed}, pid: {process.pid}"
            )
            if remote.poll(0):  # Non-blocking check
            ...
            self.log_debug(
                f"[{i}] Send completed, about to recv() process: {process.is_alive()}, remote: {remote.closed}, pid: {process.pid}"
            )
            try:
            ...
            self.log_debug(
                f"state set for remote {remote} {process.is_alive()} {time.time() - t} seconds"
            )
